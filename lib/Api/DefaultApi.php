...
class DefaultApi
{
     ...
     * Получить компанию
     *
     * @param  int $id Идентификатор компании, информацию о которой нужно получить. (required)
     * @param  string $accept e.g. application/vnd.yclients.v2+json (required)
     * @param  string $content_type application/json (required)
     * @param  string $authorization Bearer partner_token, User user_token (required)
     * @param  int $my Только для авторизованного пользователя. Если нужны дополнительные данные по компании, на управление которой пользователь имеет права (optional)
     * @param  int $for_booking Показать дату и время ближайшего свободного сеанса в компании (ISO8601). (optional)
     * @param  int $show_groups Включить в объект компании список сетей в которые входит эта компания (optional)
     * @param  int $show_bookforms Показать виджеты онлайн-записи компании (optional)
     * @param  int $bookform_id Показать адрес виджета онлайн-записи с указанным идентификатором (optional)
     ...
     * @param  string $content_type application/json (required)
     * @param  string $authorization Bearer partner_token, User user_token (required)
     * @param  int $id ID компании. Фильтр по идентификатору компании (optional)
     * @param  int $group_id ID сети компаний. Фильтр по идентификатору сети компаний _Default: 83_ (optional)
     * @param  int $my Только для авторизованного пользователя. Если нужно компании, на управление которыми пользователь имеет права (optional)
     * @param  int $active Если нужно получить только компании с активной лицензией и доступным бронированием (optional)
     * @param  int $moderated Если нужно получить только прошедшие модерацию компании, чей контент проверен для публикации (optional)
     * @param  int $for_booking Показать дату и время ближайшего свободного сеанса в компании (ISO8601) (optional)
     * @param  int $show_groups Включить в объект компании список сетей в которые входит эта компания (optional)
     * @param  int $city_id Поиск по ID города ([метод получения городов](#cities)) (optional)
     * @param  int $show_bookforms Включить в объект компании виджеты онлайн-записи (optional)
     * @param  int $vk_api_id Поиск виджеты онлайн-записи по vk_api_id. Параметр работает при showBookforms&#x3D;1 (optional)
     * @param  int $min_id Минимальный ID компании (optional)
     * @param  int $show_deleted Включить в список удалённые компании (optional)
     * @param  int $hide_record_type_single Не показывать салоны с индивидуальной записью (optional)
     * @param  int $hide_record_type_activity Не показывать салоны с групповой записью (optional)
     * @param  int $hide_record_type_mixed Не показывать салоны со смешанной записью (optional)
     * @param  int $business_group_id Идентификатор группы бизнеса. Фильтр по группе бизнеса (optional)
     * @param  int $business_type_id Идентификатор сферы бизнеса. Фильтр по сфере бизнеса (optional)
     * @param  int $yandex Фильтр по синхронизации данных компании с партнерскими площадкам (optional)
     * @param  string[] $include Включить в объект компании дополнительные данные (optional)
     * @param  int $count Количество компаний на странице (optional)
